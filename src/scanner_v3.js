(function attachScanner(globalScope) {
  ...
  const LIVE_SCAN_INTERVAL_MS = 1400;
  const INITIAL_SCAN_DELAY_MS = 0;
  const TRADING_SCAN_INTERVAL_MS = 650;
  const TRADING_INITIAL_SCAN_DELAY_MS = 30;
  ...
  function publishResult(result, source, sampleCount) {
    ...
    safeSendMessage({
      ...
      payload: {
        ...
        message:
          STATE.mode === "trading"
            ? `Fast trading data is updating on this ${STATE.selectedSource} chart.`
            : `Live ${STATE.selectedSource} data is updating on this chart.`,
        captureCount: STATE.captureCount,
        savedCount: STATE.savedCount
      }
    });
  ...
  function triggerScan(source) {
    if (!STATE.running || STATE.scanInFlight) {
      return;
    }

    STATE.scanInFlight = true;

    Promise.resolve(runScan(source))
      .catch((error) => {
  ...
  async function runScan(source = "screen-capture") {
    ...
    if (STATE.mode === "trading") {
      STATE.captureCount = Math.min(Math.max(STATE.savedCount, 1), TRAINING_CAPTURE_TARGET);
    ...
    if (STATE.captureCount < TRAINING_CAPTURE_TARGET) {
      ...
      safeSendMessage({
        ...
        payload: {
          ...
          message:
            STATE.mode === "trading"
              ? `Saved photo ${STATE.captureCount}/${TRAINING_CAPTURE_TARGET}. Trading answers start after training.`
              : `Saved photo ${STATE.captureCount}/${TRAINING_CAPTURE_TARGET} to the shared model`,
          captureCount: STATE.captureCount,
          savedCount
        }
      });
  ...
  async function start(mode = "scan") {
    try {
      if (STATE.running) {
      ...
      STATE.running = true;
      STATE.mode = mode === "trading" ? "trading" : "scan";
      STATE.lastFingerprint = "";
      STATE.lastResult = null;
      STATE.captureCount = 0;
      STATE.savedCount = 0;
      STATE.scanInFlight = false;
      await hydrateModelFromStorage();
      STATE.captureCount = Math.min(STATE.savedCount, TRAINING_CAPTURE_TARGET);

      const intervalMs =
        STATE.mode === "trading" ? TRADING_SCAN_INTERVAL_MS : LIVE_SCAN_INTERVAL_MS;
      const startDelayMs =
        STATE.mode === "trading" ? TRADING_INITIAL_SCAN_DELAY_MS : INITIAL_SCAN_DELAY_MS;
      ...
      safeSendMessage({
        ...
        payload: {
          state:
            STATE.mode === "trading"
              ? "running"
              : STATE.captureCount >= TRAINING_CAPTURE_TARGET
                ? "running"
                : "training",
          mode: STATE.mode,
          message:
            STATE.mode === "trading"
              ? "Trading mode is analyzing live without saving new photos."
              : STATE.captureCount >= TRAINING_CAPTURE_TARGET
                ? "Using the saved 10 training photos. Live analysis is ready."
                : `Collecting photo ${STATE.captureCount}/${TRAINING_CAPTURE_TARGET} for the shared model`,
          captureCount:
            STATE.mode === "trading"
              ? Math.min(Math.max(STATE.savedCount, 1), TRAINING_CAPTURE_TARGET)
              : STATE.captureCount,
          savedCount: STATE.savedCount
        }
      });
  ...
  function stop() {
    ...
    safeSendMessage({
        ...
        payload: {
          state: "idle",
          mode: STATE.mode,
          message: STATE.mode === "trading" ? "Trading mode stopped for this tab." : "Scan stopped for this tab."
        }
      });
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message?.type === "START_SCAN") {
      ...
      Promise.resolve(start(message.mode || "scan"))
